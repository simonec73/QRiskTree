...
This guidance applies to version 0.2.0 and is still valid for version 0.3.0. With version 0.4.0, I've introduced some key changes, which expand QRiskTree Editor's capabilities. You can find their detailed description below. The prefix ([0.4]) specifies when the feature has been introduced or last modified. Version 0.5.0 has not introduced any new feature. Version 0.6.0 has introduced some new important features and improved a couple of existing ones.

I'm planning to eventually extend this list in the future.

### [0.6] Encryption

It is now possible to save the models as encrypted files with extension .QRISK.

To create an encrypted file, you just have to save it as an encrypted file simply by choosing the **QRiskTree encrypted files (*.qrisk)** file type while saving it. You can do that with new files using the **Save** command in the **File** menu; with existing files you must use the **Save As...** command. This opens the dialog shown below, where you must specify the password twice to ensure it is captured correctly. It also includes a bar that is filled based on the quality of the password.

<img src="./Pictures/SaveEncrypted.png" alt="The Save As dialog" style="zoom:50%;" />

Once a file is saved as an Encrypted file, it is saved again as such every time you use the **CTRL+S** combination or you use the **Save** command. You do not have to specify again the password, as QRiskTree remembers it for you in a safe way (SecureString).

> **Note**
> The encryption capability is implemented within a new library, QRiskTree.Encryption. This library implements a [Crypto Agile](https://csrc.nist.gov/projects/crypto-agility) approach to encryption you are welcome to use with your own code. The first incarnation uses standard security at the moment of writing. This is adequate for the intent of the first version, but it is not mid-term. I'm planning to release a couple of new versions in the future: the first to get even better security using current algorithms; the second to adopt Quantum-resistant cryptography. In any case, the adoption of crypto agile patterns ensures that you will be able to open older files with every newer version of QRiskTree Editor.

### [0.6] Fact Analyzers

There are situations where you need to analyze more Facts to determine a summary metric. Fact Analyzers provide a way to do so, by defining a small tree of Montecarlo simulations. The QRiskTree Editor allows creating three levels of Fact Analyzers by using the **Fact Analyzers** table. You can find a two levels hierarchy in the picture below.

![Fact Analyzers](./Pictures/FactAnalyzers.png)

The QRiskTree library does not impose any limit on the depth of the Fact Analyzers tree.

You can have multiple Fact Analyzers tree defined in a Model. To create a new tree, you can use **Create a Fact Analyzer** command in the **Edit** menu, or use **Create a new Fact Analyzer** command in an empty space in the Fact Analyzers panel. To create a child Fact Analyzer, you must open the context menu on the parent and then click the **Add child Fact Analyzer** command. This command is present only on the first and second levels of Fact Analyzers.

Each Fact Analyzer has three main properties:

- The **Operation**, which represents the mathematical operation to be done on the samples generated by the child Fact Analyzers. It currently supports the following values: Sum, Multiply, Average, Min and Max.
- The **Opposite** flag. It is applied on the results of the Operation and changes the sign of each calculated sample.
- The **Reciprocal** flag is applied on the results of the Operation and calculates the reciprocal of each calculated sample.

You can trigger the calculation of the Fact Analyzers in two ways:

- By clicking the **Calculate all Fact Analyzers** action in the **Calculate** menu. This command focuses on the top Fact Analyzers.
- By opening the context menu on a Fact Analyzer and then clicking the **Calculate Fact Analyzer** action. This command can be executed on the selected Fact Analyzer, which can also be a child one. The results are stored in the rows statistics and it also creates a chart with the distribution of the samples.

![Fact Analyzer Calculation](./Pictures/FactAnalyzerCalculation.png)

> **Note**
>
> You can find the model used here as an example in the Samples folder: [Fact Analyzers.json](.\Samples\Fact%20Analyzers.json).

The calculation of Fact Analyzers has two main behaviors:

- If the Fact Analyzer is associated with one or more Facts, it generates samples for each Fact based on its definition and then applies the Operation. For example, if it is Sum, it adds up each sample corresponding to the same iteration. Then, if the Opposite flag is set, it changes the sign for each resulting sample; if the Reciprocal flag is set, it calculates the reciprocal for each resulting sample.
- If the Fact Analyzer is not associated with any Fact, but has one or more child Fact Analyzers, it calculates its children and then combine them using the Operation. Again, it uses Opposite and Reciprocal flags to process the resulting samples.

For example, the Fact Analyzer shown in the above picture can be read as such: 

- Calculate first the Negative Fact Analyzer by generating N samples (where N by default is 100000) each for facts Neg1 and Neg2, then sum the corresponding samples (Neg1[0] + Neg2[0], Neg1[1] + Neg2[1], and so on until the end) and finally change the sign of the samples in the resulting array.
- Calculate then the Positive Fact Analyzer by generating N samples each for facts Pos1 and Pos2, then sum the corresponding samples.
- Take the two resulting arrays and sum the corresponding samples (Positive[0] + Negative[0], ...).

Please note that the output of the Fact Analyzers is always a simple Number. In other words, it is not a Percentage, a Frequency or a Monetary value.

### [0.6] Control Types (Introduced with 0.4)
...
| Detective    | Detective controls are used to detect attack while they are in progress.<br />As such, Detective controls have no direct effect on the potential losses, but they act as triggers for Corrective controls. |
...
Control Types have an effect on the Optimization calculation:

- Combinations with Detective controls but no Corrective control are automatically rejected as detective controls have no effectiveness without a corresponding Corrective control.
- Combinations with Corrective controls but no Detective control are automatically rejected as there is nothing that would trigger the Corrective control.

These considerations are obvious when the Corrective control is automated, but they apply also when it is manual. In fact, you must have a well-defined process to get the signals that trigger such action. The communication channel can be as simple as emails, but that doesn't mean that it doesn't exist.
...
### [0.6] Auxiliary Mitigations (Introduced with 0.4)
...
#### Consequence of the Auxiliary Mitigations

The Auxiliary flag is applied to the Mitigations applied to Risks, not to the Mitigations themselves. This means that you might decide that for some Risk, a Mitigation is auxiliary to some other Mitigation, while for other Risks it is not. The effectiveness will be ignored only when the Auxiliary flag is set.
...
- **0.6.0**: Other major features improvement.
  - Extended charts to show Mode and Average.
  - Output pane is now a Markdown viewer. The output is saved as Markdown.
  - Added support for a new range type: simple Numeric range.
  - Non-editable cells are now shown with blue text.
  - Introduced the concept of Fact Analyzers, to allow performing simple Montecarlo simulations on Facts. See [[0.6] Fact Analyzers](#[0.6]-fact-analyzers).
  - Added the possibility to save the Model as an encrypted file with extension .QRISK. See [[0.6] Encryption](#[0.6]-encryption).
  - Improved Optimal Set calculations by fixing issues with the Auxiliary flag. See [[0.6] Control Types (Introduced with 0.4)](#[0.6]-control-types-introduced-with-0.4) and [[0.6] Auxiliary Mitigations (Introduced with 0.4)](#[0.6]-auxiliary-mitigations-introduced-with-0.4).
