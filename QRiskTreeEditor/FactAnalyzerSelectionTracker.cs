...
{
    public static class FactAnalyzerSelectionTracker
    {
        ...
        public static readonly DependencyProperty TrackSelectionProperty =
            DependencyProperty.RegisterAttached(
                "TrackSelection",
                typeof(bool),
                typeof(FactAnalyzerSelectionTracker),
        ...
        private static void OnTrackSelectionChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is DataGrid dataGrid && e.NewValue is true)
            {
                ...
                dataGrid.SelectionChanged += (s, args) =>
                {
                    ...
                    if (rootDataContext is IFactAnalyzerSelectionReceiver receiver)
                    {
                        receiver.SelectedFactAnalyzer = dataGrid.SelectedItem;
                    }
                };
            }
        }
        ...
        private static object? FindRootDataContext(DependencyObject current)
        {
            while (current != null)
            {
                if (current is FrameworkElement fe && fe.DataContext is IFactAnalyzerSelectionReceiver)
